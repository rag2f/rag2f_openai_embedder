...
class TestOpenAIEmbedderConfiguration:
    ...
    def test_valid_config_initializes_correctly(self):
        ...
        config = {"api_key": "sk-test-key", "model": "text-embedding-3-small", "size": 1536}
...
class TestOpenAIEmbedderContract:
    ...
    def test_client_initialized_with_correct_params(self, mock_client):
        ...
        MockClient.assert_called_once_with(api_key="sk-my-secret-key", timeout=60.0, max_retries=3)
    ...
    def test_returns_embedding_as_list(self, mock_client):
        ...
        config = {"api_key": "sk-test-key", "model": "text-embedding-3-small", "size": 3}
...
class TestOpenAIEmbedderEdgeCases:
    ...
    def test_empty_string_input(self):
        ...
        with patch(OPENAI_PATCH_TARGET) as MockClient:
            mock_instance = MagicMock()
            mock_instance.embeddings.create.return_value.data = [MagicMock(embedding=[0.0] * 1536)]
            MockClient.return_value = mock_instance

            config = {"api_key": "sk-test-key", "model": "text-embedding-3-small", "size": 1536}

            embedder = OpenAIEmbedder(config)
            _ = embedder.getEmbedding("")  # Empty string
...
class TestOpenAIEmbedderHTTPIntegration:
    ...
    def test_full_integration_smoke_test(self):
        ...
        respx.post(url__regex=r".*").mock(return_value=httpx.Response(200, json=mock_response))
